...
import { BookOpen, Zap, ArrowRight, RotateCcw, Search } from 'lucide-react';
...
const ROAD_SIGNS = [
  ...
  { id: 'm4.20.3', code: '4.20.3', name: 'Напрямок руху ТЗ з небезпечними вантажами', category: 'mandatory', image: '/images/sign_img/4.18.3z.png' },
...
const CATEGORIES = [
  { id: 'all', label: 'Всі знаки' },
  { id: 'warning', label: '⚠️ Попереджувальні' },
  { id: 'priority', label: '🔶 Пріоритет' },
  { id: 'prohibitory', label: '🚫 Заборонні' },
  { id: 'mandatory', label: '🔵 Наказові' },
  { id: 'informational', label: '🟢 Інформаційні' },
  { id: 'service', label: '🛠️ Сервіс' },
  { id: 'additional', label: '📌 Таблички' },
];
            ...
            {CATEGORIES.map(cat => (
              <button key={cat.id} onClick={() => setSelectedCategory(cat.id)}
                className={cn("px-3 py-1.5 rounded-lg text-sm font-medium border transition-all",
                  selectedCategory === cat.id ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border text-muted-foreground hover:text-foreground"
                )}>
                {cat.label}
              </button>
            ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
            {filteredSigns.map((sign, i) => (
              <motion.div key={sign.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}>
                <Card className={cn("cursor-pointer transition-all hover:shadow-md hover:border-primary/30 border", sign.color)}
                  onClick={() => setSelectedSign(selectedSign?.id === sign.id ? null : sign)}>
                  <CardContent className="p-3 text-center">
                    <img src={sign.image} alt={sign.name} className="mx-auto w-16 h-16 object-contain mb-2" />
          ...
          <AnimatePresence>
            {selectedSign && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                className="fixed bottom-4 left-1/2 -translate-x-1/2 w-full max-w-lg mx-auto px-4 z-40">
                <Card className={cn("border-2 shadow-2xl", selectedSign.color)}>
                  <CardContent className="p-5 flex items-start gap-4">
                    <img src={selectedSign.image} alt={selectedSign.name} className="w-24 h-24 object-contain shrink-0" />
                    <div className="flex-1 min-w-0">
                      <Badge variant="outline" className="text-xs">{selectedSign.code}</Badge>
                      <p className="font-bold text-foreground mt-1">{selectedSign.name}</p>
                      <p className="text-sm text-muted-foreground mt-1">{selectedSign.desc}</p>
                    </div>
                    <button onClick={() => setSelectedSign(null)} className="text-muted-foreground hover:text-foreground">✕</button>
                  </CardContent>
                </Card>
              </motion.div>
            )}
              ...
              {selectedAnswer && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  <div className={cn("p-4 rounded-xl text-sm", selectedAnswer.id === currentQuizSign.id ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700")}>
                    ...
                    <p className="mt-2 text-xs opacity-80">{currentQuizSign.desc}</p>
                  </div>
